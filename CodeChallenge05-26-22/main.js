...
//Notes
// I decided to create a variable called 'answer' which already has the first 3 array numbers that we need.
// I decided to  loop through the total number of arrays we need (n)
// i then decided to add the next 3 signature variables that and push that to answer. after the loop is finished we'll have to slice our answer from the start until n

function tribonacci(signature, n) {
  let answer = signature;
  for (let i = 0; i <= n; i++) {
    if (signature.length < n) {
      answer.push(signature[i] + signature[i + 1] + signature[i + 2]);
    }
  }
